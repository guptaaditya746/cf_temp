...
if __name__ == "__main__":
    ...
    # =====================================================
    print("\n" + "=" * 70)
    print("🔍 COUNTERFACTUAL GENERATION - NEAREST PROTOTYPE")
    print("=" * 70 + "\n")
    ...
    if result is None:
        ...
        )  # ✅ Fixed: Nearest Prototype has 'distance'
        print(f"  Prototype idx: {result['meta']['prototype_index']}")
        ...
        save_path = os.path.join(output_dir, "nearest_prototype_plot.png")

        plot_counterfactual(
            x=x.cpu().numpy(),
            x_cf=result["x_cf"].cpu().numpy(),
            edit_segment=None,  # ✅ Fixed: Nearest Prototype has NO segment (full replacement)
            feature_names=[f"sensor_{i}" for i in range(x.shape[1])],
            title=f"Nearest Prototype Counterfactual (Prototype #{result['meta']['prototype_index']})\n"
        ...
        combined_results = {
            ...
            "counterfactual_result": {
                ...
                "distance": float(
                    result["distance"]
                ),  # ✅ Fixed: use 'distance' not 'edit_segment'
                "method": result["meta"]["method"],
                "prototype_index": result["meta"]["prototype_index"],
            },
            "evaluation_metrics": make_serializable(metrics),
        }

        # Save to JSON
        json_path = os.path.join(output_dir, "nearest_prototype_summary.json")
